// Thin wrappers around @solana/web3.js + Anchor for client-side use.
// Production version of this lives in the sdk/ package; this is the
// minimal surface the Next.js app needs.
...
export const PROGRAM_ID = new PublicKey(
  process.env.NEXT_PUBLIC_PROGRAM_ID ??
    "VPasportProgram1111111111111111111111111111"
);
...
// TODO: replace with real account fetch + Anchor decode once IDL is generated.
export async function fetchVehicleEvents(_vin: string) {
  // Two viable backends:
  //   1. getProgramAccounts with filter on event.vehicle (slow for large sets)
  //   2. Helius DAS API (recommended; tiny per-call latency)
  // For v0 the writer portal can pass events forward via local cache.
  return [];
}
